    ...
    {        
        ...
        case APP_STATE_SWITCH_PRESS_WAIT:
            if (SWITCH_GET() == SWITCH_STATE_PRESSED)
            {
                ...
                if (SYS_CONSOLE_DeviceGet(SYS_CONSOLE_INDEX_1) == SYS_CONSOLE_DEV_USB_CDC)
                {
                    /* Send a message on USB console instance 1 */
                    SYS_CONSOLE_Write(SYS_CONSOLE_INDEX_1, "***This is USB Console Instance 1***", sizeof("***This is USB Console Instance 1***"));
        ...
        case APP_STATE_WAIT_READ_COMPLETE:
            ...
            if (SYS_CONSOLE_ReadCountGet(SYS_CONSOLE_INDEX_0) >= USB_CONSOLE_NUM_BYTES_READ)
            {
                SYS_PRINT("\n\rSYS_CONSOLE_INDEX_0: Free Space in RX Buffer = %d bytes", SYS_CONSOLE_ReadFreeBufferCountGet(SYS_CONSOLE_INDEX_0));   
                
                /* USB_CONSOLE_NUM_BYTES_READ or more characters are available. Read the data in the application buffer. */
                if (SYS_CONSOLE_Read(SYS_CONSOLE_INDEX_0, usb_console_0_read_buffer, USB_CONSOLE_NUM_BYTES_READ) == USB_CONSOLE_NUM_BYTES_READ)
                {
        ...
        case APP_STATE_WRITE_RECEIVED_DATA:
            ...
            SYS_CONSOLE_Write(SYS_CONSOLE_INDEX_0, usb_console_0_read_buffer, USB_CONSOLE_NUM_BYTES_READ); 
        ...
        case APP_STATE_CONSOLE_READ_WRITE:
            ...
            if (SYS_CONSOLE_Read(SYS_CONSOLE_INDEX_0, usb_console_0_read_buffer, 1) >= 1)
            {
                LED_TOGGLE();
                SYS_CONSOLE_Write(SYS_CONSOLE_INDEX_1, usb_console_0_read_buffer, 1);            
            }
            /* Read a character from USB console instance 1 and echo it on USB console instance 0*/
            if (SYS_CONSOLE_Read(SYS_CONSOLE_INDEX_1, usb_console_1_read_buffer, 1) >= 1)
            {
                LED_TOGGLE();
                SYS_CONSOLE_Write(SYS_CONSOLE_INDEX_0, usb_console_1_read_buffer, 1);            
            }
            break;
