<#if (HarmonyCore.SELECT_RTOS)?? && HarmonyCore.SELECT_RTOS == "FreeRTOS">
    <#lt>static void lDRV_SDSPI_${INDEX?string}_Tasks(  void *pvParameters  )
    <#lt>{
    <#lt>    while(true)
    <#lt>    {
    <#lt>        DRV_SDSPI_Tasks(sysObj.drvSDSPI${INDEX?string});
             <#if DRV_SDSPI_RTOS_USE_DELAY >
    <#lt>        vTaskDelay(${DRV_SDSPI_RTOS_DELAY}U / portTICK_PERIOD_MS);
             </#if>
    <#lt>    }
    <#lt>}
...
<#elseif (HarmonyCore.SELECT_RTOS)?? && HarmonyCore.SELECT_RTOS == "MbedOS">
    ...
    <#lt>        DRV_SDSPI_Tasks(sysObj.drvSDSPI${INDEX?string});
    <#if DRV_SDSPI_RTOS_USE_DELAY == true>
        <#lt>    thread_sleep_for((uint32_t)(${DRV_SDSPI_RTOS_DELAY}U / MBED_OS_TICK_PERIOD_MS));
    </#if>
    <#lt>    }
    <#lt>}
</#if>
