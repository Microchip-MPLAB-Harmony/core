/*******************************************************************************
  ...
  Description:
    ...
    microcontroller. This file implements the MEDIAFLASH Driver Erase Write function.
    This file should be included in the project if MEDIAFLASH driver Erase Write
...
)
{
    ...
    *tempHandle1 = DRV_MEDIAFLASH_COMMAND_HANDLE_INVALID;
    ...
    _DRV_MEDIAFLASH_VALIDATE_EXPR((clientObj == NULL), (void)0);
    _DRV_MEDIAFLASH_VALIDATE_EXPR((sourceBuffer == NULL), (void)0);
    _DRV_MEDIAFLASH_VALIDATE_EXPR(((writeBlockStart + nWriteBlock) > gMEDIAFLASHGeometryTable[GEOMETRY_TABLE_WRITE_ENTRY].numBlocks), (void)0);
    _DRV_MEDIAFLASH_VALIDATE_EXPR((nWriteBlock == 0), (void)0);

    /* Check if the driver was opened for write */
    _DRV_MEDIAFLASH_VALIDATE_EXPR((!(clientObj->intent & DRV_IO_INTENT_WRITE)), (void)0);

    dObj = clientObj->driverObj;
  
    writeBlockStart *= DRV_MEDIAFLASH_ROW_SIZE;
    writeBlockStart += dObj->blockStartAddress;
    nWriteBlock     *= DRV_MEDIAFLASH_ROW_SIZE;

    /* Address should be row aligned */
    _DRV_MEDIAFLASH_VALIDATE_EXPR(((writeBlockStart % DRV_MEDIAFLASH_ROW_SIZE) != 0), (void)0);

    /* Disable the interrupt so that any write operation from an interrupt 
     * context does not interfere. 
     * */
    interruptWasEnabled = _DRV_MEDIAFLASH_InterruptSourceDisable (dObj->interruptSource);
    ...
    for(iEntry = 0; iEntry < DRV_MEDIAFLASH_BUFFER_OBJECT_NUMBER; iEntry ++)
    {
        if(gDrvMEDIAFLASHBufferObject[iEntry].inUse == false)
        {
            ...
            bufferObj->flag                 = DRV_MEDIAFLASH_BUFFER_FLAG_ERASE_WRITE;
            ...
            if(dObj->writeEraseQ == NULL)
            {
                 ...
                 * object.  */
                dObj->nRowsPending = nWriteBlock/DRV_MEDIAFLASH_ROW_SIZE;
                ...
                erasePageAddress = (writeBlockStart / DRV_MEDIAFLASH_PAGE_SIZE) * DRV_MEDIAFLASH_PAGE_SIZE;
